...
// Date: 06/14/2001
// Modified by Roger Broncano Reyes 16/10/2003
//==============================================================================
unit CalcExpress;
...
function TCalcExpress.calc(args: array of extended): extended;
  ...
  begin
    ...
    case t^.num of
      ...
      27: c := log10(c(t^.l)); //rgb
      28: c := Integer(not(Boolean(Trunc(c(t^.l)))));
      31: c := power(c(t^.l), c(t^.r));
      32: c := Trunc(c(t^.l)) mod Trunc(c(t^.r)); //rgb
      33: c := Trunc(c(t^.l)) div Trunc(c(t^.r)); //rgb
      34: c := Integer(Boolean(Trunc(c(t^.l))) and Boolean(Trunc(c(t^.r)))); //rgb
      35: c := Integer(Boolean(Trunc(c(t^.l))) or Boolean(Trunc(c(t^.r)))); //rgb
      36: c := Integer(c(t^.l) = c(t^.r)); //rgb
      37: c := Integer(c(t^.l) <> c(t^.r)); //rgb
      38: c := Integer(c(t^.l) > c(t^.r)); //rgb
      39: c := Integer(c(t^.l) < c(t^.r)); //rgb
      40: c := Integer(c(t^.l) <= c(t^.r)); //rgb
      41: c := Integer(c(t^.l) >= c(t^.r)); //rgb
    end;
  end;
begin
  calc := c(Tree);
  ...
  begin
    ...
    try
      ...
      begin
        ...
        inc(pos);
        if (pos > length(s)) or not (s[pos] in ['0'..'9']) then Error('Número erróneo en la expresión.');
      ...
      Result := Result + s[pos];
      inc(pos);
      if pos > length(s) then Error('Número erróneo en la expresión.');
      if s[pos] in ['-', '+'] then
      begin
        Result := Result + s[pos];
        inc(pos);
  ...
  begin
    ...
    case s[pos] of
      ...
      begin
        ...
        if con = 'sgn' then num := 15;
        if con = 'sqr' then num := 16;
        if con = 'ln' then num := 17;
        if con = 'exp' then num := 18;
        if con = 'arcsin' then num := 19;
        if con = 'arccos' then num := 20;
        if con = 'arctan' then num := 21;
        if con = 'arcctg' then num := 22;
        if con = 'sinh' then num := 23;
        if con = 'cosh' then num := 24;
        if con = 'tanh' then num := 25;
        if con = 'ctgh' then num := 26;
        if con = 'log' then num := 27;
        if con = 'not' then num := 28;
        if con = 'mod' then num := 32;
        if con = 'and' then num := 34;
        if con = 'or' then num := 35;
...
var 
  ...
  begin
    ...
    try
      ...
      begin
        // First operand
        if not (n in [7,8,10..30]) then Error('Se esperaba un nombre de variable.');
        op := n;
        opc := c;
        if n in [7,8] then
        begin
        ...
        begin
          //Function
          getlex(s, n, c);
          if n <> 1 then Error('Se esperaba una función.');
          inc(bc);
          l := newnode;
          l^.l := gettree(s); 
          l^.num := op; 
      ...
      while n = 31 do
        ...
        if (n <> 7) and (n <> 8) then Error('Se esperaba un número.');
        r := newnode; 
        r^.num := n; 
        r^.con := c;
        res := newnode; 
...
begin
  ...
  try
    ...
    begin
      ...
      if not (n in [3,4,35..41]) then Error('Se esperaba un operador.');
      op := n;
      r := getop;
      res := allocmem(sizeof(TTree));
      res^.l := l; 
